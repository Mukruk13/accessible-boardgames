...
class AccessibilitySection(BoxLayout):
    def __init__(self, screen, **kwargs):
        ...
        self.voice_speed_slider = Slider(
            min=100, max=500, value=100, step=10, size_hint=(
                1, None), height=50)
        ...
        self.voice_volume_slider = Slider(
            min=0, max=100, value=100, step=1, size_hint=(1, None), height=50
        )
    ...
    def toggle_tts(self, instance):
        ...
        Clock.schedule_once(
            lambda dt: self.screen.speak_key(
                key, force=True), 0.1)
