...
def get_translations(lang_code: str) -> dict:
    ...
    if lang_code not in translations:
        logger.debug(
            f"Language '{lang_code}' not found, falling back to English")
...
def get_translation_value(lang_code: str, path: str) -> str | None:
    ...
    for key in keys:
        if isinstance(data, dict):
            ...
            if data is None:
                logger.debug(
                    f"Translation key '{path}' not found for language '{lang_code}'"
                )
                return None
        else:
            logger.debug(
                f"Unexpected non-dict type encountered at '{key}' for path '{path}'"
            )
            return None
    return data
